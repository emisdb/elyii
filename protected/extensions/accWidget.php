...
class accWidget extends CWidget {
...
//		echo "]";
	...
	private function displayRow($pos) 
  {
		...
		if($this->lvl==$this->level)
		{
			if(strlen($this->action)==0)
			{
				echo ("<li onmouseover='omover(\"$id\")' onmouseout='omout(\"$id\")'");
...
//		echo "<h1>iters:".$this->iters."</h1>";
		...
		{	
			...
			foreach($this->arr_coor as $key =>$value)
			{
				...
				$j=0;
				$k=-1;
				$script.="coors[".($i-1)."]=[$key, ";
				foreach($value as $kk =>$val)
				{
					if(($k==2)&&($val[0]!=2)) $script.="]";
					if($j!=0) $script.=",";
					$j++;
					if($val[0]==0) $script.="[0,[".$val[1].",".$val[2]."]]";
					if($val[0]==1) $script.="[1,[".$val[1].",".$val[2]."]";
					if($val[0]==2) $script.="[".$val[1].",".$val[2]."]";
					$k=$val[0];
				}
				if($k==2) $script.="]";
				$script.="];\n";
