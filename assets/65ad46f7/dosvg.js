 ...
 function operations(action) {
    ...
    var whats='';
    var wheres='';
     if(isNumeric(whatv)) whats='"t":'+whatv+',';
     if(isNumeric(wherev)) wheres='"e":'+wherev+',';
    if(action.substr(0,1)=="c"){
        if(wheres=='') { alert("Не указано где создавать"); return;}       
    }
    if(action=="d"){
        if(whats=='') { alert("Не указано что удалять"); return;}
         if(!confirm('Уверены, что хотите удалить элемент №'+whatv+'?')) return;
     }

    var txt='{'+whats+wheres+'"a":"'+action+'"}';
